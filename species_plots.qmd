...
// Access filtered_rows unconditionally so OJS always tracks this dependency,
// even when selected_lme_id is null.
current_row = {
  const rows = filtered_rows;
  if (selected_lme_id === null || rows.length === 0) return null;
  return rows.find(d => +d.lme_id === selected_lme_id) || null;
}
```
