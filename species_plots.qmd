...
// Build and render a descriptive sentence for the validated species
{
  ...
  if (rows.length > 0) {
    ...
    if (firstRow.plot_prefix === "appearing_species_plots_ggplot2") {
      ...
      htmlStr = `The species (<code>speciesKey</code> <a href="https://www.gbif.org/species/${speciesKey}">${speciesKey}</a>) is appearing in <strong>${minYear}</strong>.`;
      ...
      htmlStr = `The species is reappearing in <strong>${year}</strong> after <strong>${yearsAway}</strong> years.`;
    } else {
      htmlStr = `The species has an emerging trend.`;
    }
