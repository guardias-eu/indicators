...
// The emerging CSV may use either "specieskey" or "species_key" depending on
// which version of the file is present, so accept both column names.
species_data = {
  // Log raw column names from each CSV for debugging
  if (emerging_species_data.length > 0) {
    console.log("[DEBUG] emerging CSV columns:", Object.keys(emerging_species_data[0]));
    console.log("[DEBUG] emerging CSV first row:", emerging_species_data[0]);
  }
  ...
  return [
    ...emerging_species_data.map(d => ({
      ...
      weighted_em_status: +(d.weighted_em_status ?? d.score ?? 0),
      model: d.model ?? ""
    })),
...
// Build and render a descriptive sentence for the validated species and selected LME
{
  ...
  console.log("[DEBUG] sentence row:", row);
  ...
  if (row && row.species_key) {
    ...
    if (row.plot_prefix === "appearing_species_plots_ggplot2") {
      ...
      htmlStr = `The species ${speciesLink} in the <strong>${lmeName}</strong> has a total emerging score of <strong>${row.weighted_em_status}</strong>. Model used: <strong>${row.model || "unknown"}</strong>.`;
    }
