...
species_data = [
  ...emerging_species_data.map(function(d) {
    return {
      ...
      weighted_em_status: d["weighted_em_status"] !== undefined ? +d["weighted_em_status"] : null,
      model_used: d["model"] || ""
    };
  }),
...
// Build and render a descriptive sentence for the validated species and selected LME
{
  ...
  if (row && row.species_key) {
    ...
    if (row.plot_prefix === "appearing_species_plots_ggplot2") {
      ...
      // Look up score and model directly from the raw CSV data to avoid
      // any potential issues with the intermediate species_data mapping.
      const rawRow = emerging_species_data.find(function(d) {
        return (d["specieskey"] || d["species_key"]) === row.species_key
            && d["lme_id"] === row.lme_id;
      });
      const score = rawRow ? rawRow["weighted_em_status"] : row.weighted_em_status;
      const modelName = rawRow ? rawRow["model"] : row.model_used;
