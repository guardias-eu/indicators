...
```{ojs}
//| echo: false
// Build and render a descriptive sentence for the validated species
{
  const rows = species_data.filter(d => d.species_name === validated_species);
  if (rows.length > 0) {
    const firstRow = rows[0];
    const speciesKey = firstRow.species_key;
    let htmlStr;

    const speciesLink = `<a href="https://www.gbif.org/species/${speciesKey}">${validated_species}</a>`;

    if (firstRow.plot_prefix === "appearing_species_plots_ggplot2") {
      // Use the earliest first_year across all LME rows for this species
      const minYear = Math.min(...rows.map(d => d.first_year).filter(y => y !== null));
      htmlStr = `The species ${speciesLink} is appearing in <strong>${minYear}</strong>.`;
    } else if (firstRow.plot_prefix === "reappearing_species_plots_ggplot2") {
      const year = firstRow.reappearance_year;
      const yearsAway = firstRow.years_without_occurrences;
      htmlStr = `The species ${speciesLink} is reappearing in <strong>${year}</strong> after <strong>${yearsAway}</strong> years.`;
    } else {
      const emScore = firstRow.weighted_em_status;
      const model = firstRow.model;
      htmlStr = `The species ${speciesLink} has a total emerging score of <strong>${emScore}</strong>. Model used: <strong>${model}</strong>.`;
    }
