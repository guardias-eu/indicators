...
```{ojs}
//| echo: false
// Named reactive variable: the data row for the currently selected species + LME.
// Using a named cell ensures OJS properly tracks the dependency on both
// filtered_rows (which updates when the species changes) and selected_lme_id
// (which updates on map click or species change via reset_lme).
current_row = {
  if (selected_lme_id === null) return null;
  return filtered_rows.find(d => +d.lme_id === selected_lme_id) || null;
}
```
