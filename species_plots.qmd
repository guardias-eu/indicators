...
{
  // Find the row matching both species and currently selected LME
  const row = selected_lme_id !== null
    ? species_data.find(d => d.species_name === validated_species && +d.lme_id === selected_lme_id)
    : null;
  ...
  if (row) {
    ...
    if (row.plot_prefix === "appearing_species_plots_ggplot2") {
      ...
      htmlStr = `The species ${speciesLink} in the <strong>${lmeName}</strong> has a total emerging score of <strong>${row.weighted_em_status}</strong>. Model used: <strong>${row.model}</strong>.`;
    }
