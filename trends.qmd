...
3. Click on a species name to view its indicator plots.
...
// Display appearing species table
{
  if (selected_lme_id !== null && filtered_appearing.length > 0) {
    ...
    display(html`<p style="text-align: center; color: #999; padding: 20px;">No appearing species data available for this LME</p>`);
  }
}
```
...
// Display reappearing species table
{
  if (selected_lme_id !== null && filtered_reappearing.length > 0) {
    ...
    display(html`<p style="text-align: center; color: #999; padding: 20px;">No reappearing species data available for this LME</p>`);
  }
}
```
