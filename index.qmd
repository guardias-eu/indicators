...
// Display emerging species table
{
  if (selected_lme_id !== null && filtered_emerging.length > 0) {
    const emerging_numeric = filtered_emerging.map(d => ({
      ...d,
      weighted_em_status: d.weighted_em_status !== "" ? +d.weighted_em_status : null
    }));
    const table = Inputs.table(emerging_numeric, {
      ...
      format: {
        ...
        species_name: (value, idx) => {
          const row = emerging_numeric[idx];
          const link = document.createElement("a");
          link.href = "#";
          link.textContent = value;
          link.style.cssText = "cursor:pointer;text-decoration:underline;";
