...
Showing **${filtered_lme_ids.length}** LME region(s) for **${validated_species}**
...
```{ojs}
// Layer group that holds the LME polygons (cleared and rebuilt reactively)
lme_layer = L.layerGroup().addTo(map)
```

```{ojs}
// Build the raw-GitHub URL for a pre-generated indicator PNG.
// Filename pattern used by emtrends: lme_{lme_name}_species_{species_key}.png
// encodeURIComponent encodes spaces and special characters (e.g. space → %20).
const PLOTS_BASE_URL =
  "https://raw.githubusercontent.com/guardias-eu/emtrends/main/" +
  "data/output/indicators_plots/indicators_plots_png/";
