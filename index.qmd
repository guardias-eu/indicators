...
::: {.callout-warning}
Marine occurrence data are substantially less well represented in open biodiversity databases than terrestrial data. This means that species listed as newly appearing or emerging may in reality could have been present and spreading for considerably longer, with earlier records remaining unpublished or scattered across grey literature and personal datasets. Because of these data limitations, the current results should be interpreted with caution.
:::
...
// Display appearing species table
{
  if (selected_lme_id !== null && filtered_appearing.length > 0) {
    const table = Inputs.table(filtered_appearing, {
      ...
      format: {
        specieskey: x => htl.html`<a href="https://www.gbif.org/taxon/${x}" target="_blank">${x}</a>`,
        species: (value, idx) => {
